...
Procedure CreateComponentXTAL_TSX_3225(Zero : integer);
Var
...
Begin
    Try
        ...
        NewPcbLibComp.Name := 'XTAL_TSX_3225';
        NewPCBLibComp.Description := 'Crystal, Dual Flat No-Lead (DFN 4 Pins), 2.00 mm pitch; 4 pin, 3.20 mm L X 2.50 mm W X 0.60 mm H body';
        NewPCBLibComp.Height := MMsToCoord(0.6);

        CreateSMDComponentPad(NewPCBLibComp, '1', eTopLayer, -1.1, 0.8, 0, 0, eRectangular, 1.4, 1.15, 180, 0, 0, 0, True, True);
        CreateSMDComponentPad(NewPCBLibComp, '2', eTopLayer, -1.1, -0.8, 0, 0, eRectangular, 1.4, 1.15, 180, 0, 0, 0, True, True);
        CreateSMDComponentPad(NewPCBLibComp, '3', eTopLayer, 1.1, -0.8, 0, 0, eRectangular, 1.4, 1.15, 0, 0, 0, 0, True, True);
        CreateSMDComponentPad(NewPCBLibComp, '4', eTopLayer, 1.1, 0.8, 0, 0, eRectangular, 1.4, 1.15, 0, 0, 0, 0, True, True);

        CreateComponentTrack(NewPCBLibComp, -2.05, -1.63, -2.05, 1.63, eMechanical15, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, -2.05, 1.63, 2.05, 1.63, eMechanical15, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 2.05, 1.63, 2.05, -1.63, eMechanical15, 0.05, False);
        CreateComponentTrack(NewPCBLibComp, 2.05, -1.63, -2.05, -1.63, eMechanical15, 0.05, False);
        ...
        CreateComponentTrack(NewPCBLibComp, -1.68, -1.33, -1.68, 1.33, eMechanical11, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -1.68, 1.33, 1.68, 1.33, eMechanical11, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 1.68, 1.33, 1.68, -1.33, eMechanical11, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 1.68, -1.33, -1.68, -1.33, eMechanical11, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -1.98, 0.52, -1.98, -1.56, eTopOverlay, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, -1.98, -1.56, 1.98, -1.56, eTopOverlay, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 1.98, -1.56, 1.98, 1.56, eTopOverlay, 0.12, False);
        CreateComponentTrack(NewPCBLibComp, 1.98, 1.56, -0.94, 1.56, eTopOverlay, 0.12, False);
...
Begin
    ...
    CreateComponentXTAL_TSX_3225(0);
